...
def create_CSSProperty(prop_type=False):
    result = """
class CSSProperty(TypedDict, total=False):
"""
    if prop_type:
        result = """class CSSPropertyTypes:
"""
    kwlist = set()
    kw_val_dict = dict()
...
def summate():
    return f"""# Autogenerated with `webdata` and `webtype_gen` v2
...
{create_CSSProperty()}
{create_CSSProperty(prop_type=True)}
"""
